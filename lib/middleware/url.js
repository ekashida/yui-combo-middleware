    ...
    return function (req, res, next) {
        ...
        for (i = 0; i < groups.length; i += 1) {
            ...
            if (name === 'core' || name === 'root' || name === 'gallery') {
                ...
                // Support for `path` groups. `path` groups can either come in
                // one piece where the full path is the module name, or in two
                // pieces where a root path is given (as the version) alongside
                // a path relative to it (as the module name).
                //
                // One piece:   "kamen/rider/fourze"
                // Two pieces:  "kamen/rider", "fourze"
                for (j = 0; j < modules.length; j += 1) {
